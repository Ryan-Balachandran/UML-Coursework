...
open Str;;	(*** for split ***)	(*******************************************************************
					   ...
					   top-level interpreter, you have to say
     						   #load "str.cma";;
					   If you are generating an executable from the shell, you have to say
...
let member e l = exists ((=) e) l;;	(*** is e a member of list l? ***)
...
(***************************************************************************************
   Return all elements of l that are not in excluded. Assume that both lists are sorted 
****************************************************************************************)
...
let rec suffix sym lst =
	match lst with
	| []   -> []
	| h::t -> if h = sym then lst	(*** structural equivalence ***)
		  else suffix sym t;; 
...
let get_knowledge gram : knowledge =
	...
	let right_contexts : right_context list = map (fun s -> get_right_context s gram) nts in
	let rec helper kdg =
		let update : symbol_knowledge -> symbol_productions -> right_context -> symbol_knowledge = fun old_sym_kdg sym_prods sym_right_context ->
...
type parse_tree =		(*** among other things, parse_trees ***)
	| PT_error		(*** are the elements of the attribute stack **)
	| PT_id of string
	| PT_num of string
	| PT_term of string
...
(*******************************************************************
Double-index to find prediction (list of RHS symbols) 
For nonterminal nt and terminal t
...
Parse program according to grammar
	[Comment-out code would print predictions 
	and matches (imperatively) along the way]
Return parse tree if the program is in the language; PT_error if its not
*************************************************************************)
let parse (parse_tab:parse_table) (program:string) : parse_tree =
	...
	let rec helper pstack tokens astack =
		match pstack with
		| [] 		       -> if tokens = [] then hd astack		(*** assert: astack is nonempty ***)
		...
		| PS_sym(tos)::ps_tail ->
			...
			| (term, tok)::more_tokens ->
				if is_terminal tos gram then		(*** if tok is an individual identifier ***) 
					if tos = term then		(*** or number, term will be a generic ***)
						begin			(*** "id" or "num" ***)
							(******************************************************
					...
					| PA_error -> die ("no prediction for " ^ tos ^ " when seeing " ^ tok)
					| PA_prediction (rhs) ->
						begin
							(******************************************************
							print_string ("   match " ^ tos);

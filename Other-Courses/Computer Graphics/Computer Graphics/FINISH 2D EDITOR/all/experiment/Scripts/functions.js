...
//           {
//                if(s.ctx.isPointIn(startX, startY))
//                {
//                     s.selectedShape = s.shapeArray[i];
//                     console.log(s.selectedShape);
//                     s.isDrag = true;
//                     console.log("MOUSE OVER LINE");
//                     s.selectedShape.fillStyle = "blue";
//                     s.shapeArray[i].draw();
//                }
//                else
//                {
//                     s.selectedShape.fillStyle = "red";
//                }
//           }
//      }
     ...
     s.canvas.onmousedown = (e) => {
          ...
          s.shapeArray.forEach(shape => {
               ...
               if(shape.showHitbox === true)
               {
                    ...
                    {
                         if(Math.sqrt(Math.pow(px - handle[i].x, 2) + Math.pow(py - handle[i].y, 2)) < 10)
                         {
                              s.isDrag = true;

                              // on mouse down, take offset of mouse to point to grab anywhere within handle?
                              console.log(i);
                              if(i === 0)
                              {
                                   // s.ctx.save();

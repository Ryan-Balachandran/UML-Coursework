onload = () => {
     ...
     s.copiedShape        = null;  // copied shape to be pasted
     ...
     s.selectedShape      = null;  // current shape selected by mouse
     s.selectedShapeIndex = null;  // index of selected shape in shapeArray
     ...
     s.canvas.addEventListener('mousedown', e => { s.mouse_down(e) });
     s.canvas.addEventListener('mouseup', e => { s.mouse_up(e) });
     s.canvas.addEventListener('mousemove', e => { s.mouse_move(e) });
     ...
     {
          s.transformSelect.value = 'none';      // make sure we are not selecting any shapes while drawing
          s.noneMode();
     ...
     // UNDO LAST OPERATION <----------------------------- UNDO / REDO TRANSFORMATION ACTION?, hide previous shape iteration after making transform
     ...
     s.menubtns[6].onclick = function () 
     {
          ...
          // s.copyArray = { ...s.shapeArray[s.selectedShapeIndex] };
          // s.copyArray = { ...singleSelect };
          

          if(s.selectedShapeIndex === null)
          {
               // Give an error if a shape is not selected
               swal("ERROR!", "NO SHAPE IS SELECTED", "error");
          }
          else
          {
               s.copiedShape  = { ...s.shapeArray[s.selectedShapeIndex] };

               console.log("SHAPE[0]: ", s.shapeArray);
               console.log("COPY: ", s.copiedShape );
          }
     }
     ...
     s.menubtns[7].onclick = function () 
     {
          ...
          switch(s.copiedShape.type)
          {
               case "Line":
                    s.shapeArray.push(s.copiedShape);
               ...
               case "Polyline": 
                    s.copyArray.forEach(point => {
                         // offset each point in the array
                    });
                    console.log("8");
               break;

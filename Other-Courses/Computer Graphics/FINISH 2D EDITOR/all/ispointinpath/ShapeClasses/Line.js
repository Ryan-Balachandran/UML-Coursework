class Line extends Shape
{
     ...
     select()
     {
          ...
          circle.arc(this.x1, this.y1, this.handleSize, 0, 2 * Math.PI, false);
          s.ctx.fillStyle = 'red';
          s.ctx.strokeStyle = 'red';
          // s.ctx.globalAlpha = 0.8;
          s.ctx.fill(circle);
          ...
          s.canvas.onmousemove = (e) => {
               if(s.ctx.isPointInStroke(e.offsetX, e.offsetY))
               {
          ...
          //      for(let i = 0; i < this.handles.length; i++)
          //      {
          //           if(Math.sqrt(Math.pow(startX - handles[i].x, 2) + Math.pow(startY - handles[i].y, 2)) < 10)
          //           {

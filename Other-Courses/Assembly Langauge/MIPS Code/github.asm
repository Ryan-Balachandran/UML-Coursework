...
saveReg:	.word 0:3
		...
		.text
main:		      
	li	$t0, 0x400		# initialize LOC to starting address
	sw 	$t0, LOC		
	la	$t1, inArr		# load address of inArr and symtab
	la	$t9, symTab
		
input:		    
	la 	$a0, string		# ask user for input
	li 	$v0, 4
	syscall
...
inputEnd:		  
	la	$a0, inArr		# set the index of the input back to the first element and begin checking
	j 	checkVar
				
checkVar:     
	lw	$t5, ($a0)		# use 3 temp register to analyze each input token
	lw	$t6, 4($a0)
	lw	$t7, 8($a0)
...
noVar:      
	addi	$a0, $a0, 12		# if no label or variable found, then increment to next entry
	j	checkVar
		
isVar:		   
	li	$t2, 0x3a		# Check to see if the next is a colon, if it is, then it must be a label
...
setLabel:	    
	li	$t7, 0x00000031		# Sets to Label status to ASCI "1" for true
	jr	$ra		
				
operator:     
	lb	$t5, 20($a0)		# load next entry type
...
addtoSymTab:  
	sw	$t5, ($t9)		# store 1 word at a time to make formatting easier in the end
	sw	$t6, 4($t9)
...
beginPrint:	  
	la	$t9, symTab		# Prints header of the printout
	la	$a0, string2	
	li	$v0, 4
	syscall
...
inArrClear:	  
	addi	$t0, $t0, 4		# increment the LOC by 1
	la	$a0, inArr		# scrubs the inArr
	la	$t0, ($a0)
	addi	$t0, $t0, 100
		
clear:		    
	sb	$zero, ($a0)		# stores a null byte at each location 
	addi	$a0, $a0, 1
	bne	$a0, $t0, clear
	b 	main

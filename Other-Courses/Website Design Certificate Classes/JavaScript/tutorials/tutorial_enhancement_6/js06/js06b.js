...
subButton.addEventListener("keyup", validateName);
...
window.addEventListener("load", function() {
   ...
   let name = payment.elements.cardName;

   validateMonth();
   validateYear();

   // Select name sections input when form opens
   name.focus();
});
...
function validateName() {
   ...
   if (cardName.validity.valueMissing) {
      cardName.setCustomValidity("You forgot to enter your full name");
...
// Check that a month is selected for the expiraction date
...
function validateForm() {
   if(!validateName() || !validateCard() || !validateNumber() || !validateMonth() || !validateYear || !validateCVC) {
      submitError.style.display = 'block';
      submitError.innerHTML = 'Please fix any error(s) to submit';
